...
The Hono plugin simplifies integrating ALTCHA into your application. It provides routes and middleware for challenge generation and verification.
...
* Sets `c.var.altcha: AltchaResult`, which includes the challenge data and verification result.
...
[`/examples/hono-example.ts`](/examples/hono-example.ts)
...
```ts
import { create, deriveHmacKeySecret, randomInt, CappedMap } from 'altcha-lib/frameworks/hono';
